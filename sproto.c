...
import_type(struct sproto *s, struct sproto_type *t, const uint8_t * stream) {
	...
	int last = -1;
	t->field_n = n;
	t->fid = pool_alloc(&s->memory, sizeof(struct field) * n);
	for(i = 0; i < n; i++)
	{
		struct field *f = &t->fid[i];
...
void sproto_dump(struct sproto *s) 
{
	...
	printf("begin sproto_dump\n-------------------------------\n");
	printf("types %d  \n", s->type_n);
	for(int i = 0; i < s->type_n; i++)
	{
		struct sproto_type *p_type = &s->type[i];
		printf("#%d typename:%s\n", i, p_type->name);
		for(int j = 0; j < p_type->field_n; j++)
		{
			char array[2] = { 0, 0 };
			const char * typename = NULL;
			struct field *f = &p_type->fid[j];
			...
			printf("\t#%d %8s %1s(%-6s)\n", f->tag, f->name, array, typename);
		}
	}
	printf("protocol %d\n", s->protocol_n);
	for(int i = 0; i < s->protocol_n; i++)
	{
...
findtag(struct sproto_type *st, int tag) {
	...
	{
		tag -= st->base;
		if(tag < 0 || tag >= st->field_n)
			return NULL;
		return &st->fid[tag];
	}
	int begin = 0, end = st->field_n;
	while(begin < end)
	{
		int mid = (begin + end) / 2;
		struct field *f = &st->fid[mid];
		int t = f->tag;
		if(t == tag)
		{
			return f;
...
// 	assert(cb == EncodeCallback);
	...
	uint8_t * header = buffer;
	uint8_t * data;
	int header_sz = SIZEOF_HEADER + p_type->maxn * SIZEOF_FIELD;
	if(size < header_sz)
		return -1;
	...
	int index = 0;
	int lasttag = -1;
	for(int i = 0; i < p_type->field_n; i++)
	{
		struct field *fld = &p_type->fid[i];
		int value = 0;
		int sz = -1;
		...
		else
		{
			...
			case SPROTO_TBOOLEAN:
			{
				...
				sz = cb(ud, fld->name, fld->type, 0, NULL, &u, sizeof(u));
				if(sz < 0)
					return -1;
				if(sz == 0)
					continue;
			...
			case SPROTO_TSTRING: {
				sz = encode_string(cb, ud, fld, data, size);
				break;
			}
			case SPROTO_TSTRUCT: {
				sz = encode_struct(cb, ud, fld, data, size);
				break;
			}
			}
		}
